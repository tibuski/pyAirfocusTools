...
# Map Airfocus item colors to terminal ANSI color names
...
_group_registry: Dict[
    str, Dict[str, Any]
] = {}  # User Groups (Global Teams) from config
...
def load_registries(verify_ssl: bool = True):
    ...
    user_groups_response = make_api_request(
        "/api/team/user-groups/search", method="POST", data={}, verify_ssl=verify_ssl
    )
...
def get_all_workspaces(verify_ssl: bool = True) -> list:
    """
    Return all workspaces from the registry (no additional API calls).
    Ensures registries are loaded first.
...
def get_workspace_id_from_name(workspace_name: str, exact_match: bool = True) -> str:
    ...
    if len(matches) > 1:
        ...
        raise Exception(
            f"Multiple workspaces found matching '{workspace_name}':\n{match_list}\nPlease use the exact workspace ID instead."
        )
...
def build_workspace_hierarchy(
    workspaces: list, verify_ssl: bool = True
) -> Dict[str, Any]:
    """
    ...
    def build_node(ws_id: str, visited: set = None) -> Dict[str, Any]:
        ...
        if ws_id in visited:
            ...
            print(
                f"Warning: Circular reference detected for workspace '{ws_name}' ({ws_id}) - workspace references itself as its own child"
            )
            return {"workspace": workspace_map[ws_id], "children": []}

        visited.add(ws_id)

        node = {"workspace": workspace_map[ws_id], "children": []}
    ...
    return {"roots": roots, "map": node_map}
...
def get_unique_members_by_prefix(
    prefix: str, exclude_suffix: Optional[str] = None
) -> set:
    """
...
def get_groups_matching_pattern(
    prefix: str, exclude_suffix: Optional[str] = None
) -> list:
    """
...
def get_users_not_in_specific_groups(
    prefixes: list, exclude_suffix: Optional[str] = None, role: Optional[str] = None
) -> set:
    """
...
def build_folder_hierarchy(workspaces: list, verify_ssl: bool = True) -> Dict[str, Any]:
    ...
    orphaned_workspace_ids = [
        ws_id for ws_id in workspace_map.keys() if ws_id not in workspace_to_folder
    ]
        ...
        for ws_id, f_id in workspace_to_folder.items():
            if f_id == folder_id and ws_id in workspace_map:
                node["workspaces"].append(
                    {"workspace": workspace_map[ws_id], "children": []}
                )
    ...
    for ws_id in orphaned_workspace_ids:
        if ws_id in workspace_map:
            roots.append({"workspace": workspace_map[ws_id], "children": []})

    return {"roots": roots, "folder_map": folder_map}
...
def get_field_id_by_name(field_name: str, registries=None, verify_ssl=True) -> str:
    ...
    fields_response = make_api_request(
        "/api/fields/search", method="POST", data={}, verify_ssl=verify_ssl
    )
    fields = (
        fields_response.get("items", [])
        if isinstance(fields_response, dict)
        else fields_response
    )
    for field in fields:
...
def get_field_by_name(field_name: str, verify_ssl=True):
    ...
    for field in fields:
        if field.get("name") == field_name:
            ...
            full_field = make_api_request(
                f"/api/fields/{field_id}", verify_ssl=verify_ssl
            )
            return full_field
    return None
...
def get_user_workspaces(user_id: str, verify_ssl: bool = True) -> list:
    ...
    """
    # Reuse the workspace registry instead of re-fetching
    all_workspaces = get_all_workspaces(verify_ssl=verify_ssl)
...
def get_extension_app_id(extension_type: str, verify_ssl: bool = True) -> Optional[str]:
    ...
    try:
        ...
        raise Exception(
            f"Failed to fetch extension app ID for type '{extension_type}': {e}"
        )
...
def install_workspace_extension(
    ...
    try:
        response = make_api_request(
            endpoint, method="POST", data=request_body, verify_ssl=verify_ssl
        )
        return response
    except Exception as e:
